...
void FuzzyChoquetIntegral::process(const cv::Mat &img_input, cv::Mat &img_output, cv::Mat &img_bgmodel)
{
  ...
  else
  {
    ...
    fu.SimilarityDegreesImage(lbp_input_f1, lbp_background_f1, sim_texture_f1, 1, colorSpace);// Similarity measre for color

    IplImage* sim_color_f3 = cvCreateImage(cvSize(input_f3->width, input_f3->height), IPL_DEPTH_32F, 3);
    fu.SimilarityDegreesImage(input_f3, background_f3, sim_color_f3, 3, colorSpace);// Similarity measre for textre	
    ...
    if(showOutput)
    {
	  ...
	  //bool imwrite(const string& "E:\My Doc\bgslibrary - master\op", InputArray img_foreground_u1, const vector<int>& CV_IMWRITE_PNG_COMPRESSION = vector<int>());
	  //cv::imwrite("E:\My Doc\bgslibrary - master\op", img_foreground_u1);
    }
